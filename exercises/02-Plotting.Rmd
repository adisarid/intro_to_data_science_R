...
Try to figure out what do they mean, and think about how would you solve the problem. 

Don't dig too deep into the matter! (this exercise is about visualizations not data import), just read the output of the `read_csv` function (which indicates the parsing failures), and think of a quick way to avoid the problem.

The error message contains a row location so you can look at the data in the area of the error to try and figure it out, e.g.: 
...
Reading the plot you generated, answer:
...
google_play <- google_play %>%
   mutate(size_app_numeric = str_replace(XXX, "k", "")) %>% 
   mutate(size_app_numeric = str_replace(XXX, "M", "000")) %>% 
