...
appointments_raw <- read_csv("https://raw.githubusercontent.com/adisarid/intro_to_data_science_R/master/datasets/Medical_Appointments_No_Shows_KaggleV2-May-2016.csv")
appointments <- appointments_raw %>%
  ...
  mutate_at(.vars = vars("schedule_time", "Age"), ~{((. - min(.))/(max(.)-min(.)))}) %>%
  select(XXX)
